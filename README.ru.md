...
- редактирование YAML со сворачиваемыми секциями, dry-run и применение;
- Describe, Events, Problems и Related Resources с поддержкой Ingress, Traefik IngressRoute и маршрутов Gateway API;
- эндпоинты сервиса из EndpointSlice в сводке ресурса;
...
- история потребления CPU и RAM подов, записываемая во время работы KubeDeck, с линиями request и limit;
- необязательный анализ через OpenAI-совместимую локальную LLM с передачей истории потребления для оценки request/limit, в которую никогда не передаются Kubernetes-логи;
...
Собранные пакеты публикуются на
[странице релизов](https://github.com/maksimvpronin-byte/kubedeck/releases):
установщик и portable-исполняемый файл для Windows, DMG и ZIP для macOS Apple
Silicon, AppImage для Linux x64. Каждый пакет собирает
`.github/workflows/release.yml` по тегу `v<версия>` теми же скриптами, что и
ручная сборка.

**About - Обновления** обращается к этой странице и показывает результат.
Ничего не скачивается, пока об этом не попросят. Двум сборкам сообщается только
о наличии новой версии, потому что заменить себя они не могут: portable-файлу
для Windows, которому нечего заменять, и неподписанной сборке macOS, которую
macOS не даст обновить на месте. Обе отправляются на страницу релизов - см.
...
```text
apps\desktop\release\KubeDeck-Setup-2.28.1-x64.exe
apps\desktop\release\KubeDeck-Portable-2.28.1-x64.exe
```
...
```text
apps/desktop/release/KubeDeck-2.28.1-arm64.dmg
apps/desktop/release/KubeDeck-2.28.1-arm64.zip
```
...
```text
apps/desktop/release/KubeDeck-2.28.1-x86_64.AppImage
```
...
- [Release notes 2.28.1](./docs/releases/RELEASE_NOTES_2.28.1.md)
- [Regression checklist 2.28.1](./docs/releases/REGRESSION_CHECKLIST_2.28.1.md)
