...
- Unchecking a namespace in the selector no longer drops it back into the alphabetical list under the cursor. Every namespace touched while the menu is open stays in a block at the top, checked or not, separated from the rest, so re-checking one does not mean finding it again; reopening the menu resets the block to the actual selection.
