...
- Fixed node usage charts taking a long time to appear. The resource list route awaited `kubectl get` before starting `kubectl top`, so Nodes, Pods and Namespaces paid for two kubectl round trips in sequence; the metrics command and the resource quota lookup now run alongside the list, in the cluster overview as well. Per-node disk usage, which costs one `kubectl get --raw .../stats/summary` process per node, now fans out to six concurrent requests instead of two and coalesces its row updates into one table update.
