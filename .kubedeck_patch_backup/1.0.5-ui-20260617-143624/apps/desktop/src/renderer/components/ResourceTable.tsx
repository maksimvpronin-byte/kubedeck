      ...
      <div className="resource-table-header">
        ...
        <div className="resource-table-actions">
          {nodeActionsVisible ? (
            <>
              {onBulkCordon ? (
                <button className="secondary-btn" type="button" onClick={() => onBulkCordon(selectedRows)} disabled={loading}>
                  Cordon ({selectedRows.length})
                </button>
              ) : null}
              {onBulkUncordon ? (
                <button className="secondary-btn" type="button" onClick={() => onBulkUncordon(selectedRows)} disabled={loading}>
                  Uncordon ({selectedRows.length})
                </button>
              ) : null}
              {onBulkDrain ? (
                <button className="danger-btn" type="button" onClick={() => onBulkDrain(selectedRows)} disabled={loading}>
          ...
          {onBulkDelete && selectedRows.length > 0 ? (
            <button className="danger-btn" type="button" onClick={() => onBulkDelete(selectedRows)} disabled={loading}>
          ...
          <button className="secondary-btn" type="button" onClick={onRefresh} disabled={loading}>
              ...
              {visibleColumns.map((column) => (
                <th key={column.key} style={{ width: widthFor(column) }}>
                  <button type="button" className="table-sort-button" onClick={() => changeSort(column.key)}>
                    {column.label}{sortKey === column.key ? (sortDirection === 1 ? " в–І" : " в–ј") : ""}
                  </button>
