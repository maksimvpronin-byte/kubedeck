...
test("KubeDeck release keeps the runtime and build pipeline Node-only", () => {
  ...
  const desktopPackage = JSON.parse(read("apps/desktop/package.json").replace(/^\uFEFF/, ""));
  const mainSource = read("apps/desktop/src/main/main.ts");
  const gatewaySource = read("apps/desktop/src/main/backend/gateway.ts");
  const migrationSource = read("apps/desktop/src/main/backend/routes/migrationStatus.ts");
  const clustersSource = read("apps/desktop/src/main/backend/routes/clusters.ts");
  ...
  const { routeOwnershipSummary } = require("../dist/main/backend/routeOwnership.js");
  ...
  assert.match(desktopPackage.scripts["test:gateway"], /node-only-runtime\.contract\.test\.cjs/);

  assert.equal(fs.existsSync(path.join(repoRoot, "apps/backend")), false);
  assert.equal(fs.existsSync(path.join(repoRoot, "apps/desktop/src/main/backend/legacyProxy.ts")), false);
  assert.equal(fs.existsSync(path.join(repoRoot, "apps/desktop/src/main/backend/legacyControl.ts")), false);

  for (const source of [mainSource, gatewaySource, migrationSource, clustersSource, yamlSource]) {
  ...
  assert.doesNotMatch(builderSource, /PyInstaller|pip\s+install|requirements\.txt|apps[\\/]backend|\.build-venv|\bpy\s+-3\b/);
  ...
  assert.doesNotMatch(setupSource, /Python\.Python|pip\s+install|apps[\\/]backend|\bpy\s+-3\b/);
