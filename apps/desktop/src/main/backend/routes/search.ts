...
function normalizeSearchQuery(value: string): string {
  ...
  if (query.length < 2) {
    throw new RequestValidationError(400, "SEARCH_QUERY_TOO_SHORT", "Search query must contain at least 2 characters");
  }
  if (query.length > SEARCH_QUERY_MAX_CHARS) {
    throw new RequestValidationError(400, "SEARCH_QUERY_TOO_LONG", `Search query must be at most ${SEARCH_QUERY_MAX_CHARS} characters`);
  }
  return query;
}
...
function parseLimit(value: string | null): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 500) {
    throw new RequestValidationError(400, "INVALID_SEARCH_LIMIT", "Search limit must be an integer between 1 and 500");
  }
  return parsed;
}
...
function parseBoolean(value: string | null, fallback: boolean): boolean {
  ...
  throw new RequestValidationError(400, "INVALID_BOOLEAN", "includeCrdInstances must be true or false");
}
...
function requestOptions(request: IncomingMessage): SearchOptions {
  ...
  return {
    ...
    includeCrdInstances: parseBoolean(url.searchParams.get("includeCrdInstances"), true),
  };
}

export function matchSearchRoute(method: string | undefined, pathname: string): SearchTarget | null {
...
async function searchSource(
  ...
  try {
    for (const namespaceMode of namespaceModes) {
      const data = await runner.runJson(clusterCommand(configStore, clusterId, resourceArgs(spec, namespaceMode), SEARCH_KUBECTL_TIMEOUT_SECONDS, SEARCH_MAX_OUTPUT_BYTES), signal);
      const items = asItems(data);
      rawCount += items.length;
      collected.push(...rankRawItems(spec, items, query, Math.max(0, limitPerResource - collected.length)));
...
async function collectSearchSources(
  ...
  let index = 0;
  let completed = false;
  const workers = Array.from({ length: Math.min(Math.max(1, concurrency), Math.max(1, specs.length)) }, async () => {
    while (true) {
      const current = index;
      index += 1;
...
export async function buildSearchResponse(
  ...
  const completed = await collectSearchSources(
    ...
    async (spec) => {
      ...
      const source = await searchSource(configStore, runner, clusterId, spec, options.namespaces, options.query, limitPerResource, controller.signal);
  ...
  const ranked = deduplicateSearchResults(results).sort(compareSearchResults).slice(0, options.limit);
  return {
    items: ranked,
    summary: {
      query: options.query,
