    ...
    <>
      {nodeAction ? (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) props.onCloseNodeAction();
          }}
        >
          <div className="node-action-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="node-action-confirm-title" onMouseDown={(event) => event.stopPropagation()}>
            <div className="node-action-confirm-header">
              ...
              <button className="icon-button" type="button" aria-label="Close" onClick={props.onCloseNodeAction}>
                <X size={16} />
              </button>
            </div>
            ...
            {nodeAction.action === "drain" ? (
              <div className="node-action-confirm-section">
                ...
                {nodeAction.previewLoading ? (
                  <p className="node-drain-preview-muted">Loading pods on selected nodes...</p>
                ) : nodeAction.previewError ? (
                  <pre className="node-action-confirm-command node-drain-preview-error">{nodeAction.previewError}</pre>
                  ...
                  <div className="node-drain-pod-list">
                    {nodeAction.affectedPods.map((pod) => (
                      <div className="node-drain-pod-row" key={resourceIdentityLabel(pod)}>
                        <code>
                          {String(pod.namespace ?? "_cluster")}/{String(pod.name)}
                        </code>
            ...
            <div className="node-action-confirm-actions modal-actions">
              <button className="secondary" type="button" onClick={props.onCloseNodeAction}>
                Cancel
              </button>
              <button className="primary" type="button" onClick={props.onConfirmNodeAction}>
                Confirm
              </button>
            </div>
          </div>
        ...
        <div className="modal-backdrop" role="presentation">
          <section className="confirm-modal bulk-delete-modal" role="dialog" aria-modal="true" aria-labelledby="bulk-delete-title">
            <header>
              ...
              <button className="icon-button" onClick={props.onCloseBulkDelete} title={t("common.close")}>
                <X size={16} />
              </button>
            </header>
            <div className="confirm-body">
              <p>
                {t("bulkDelete.text")} <strong>{bulkDelete.rows.length}</strong>. {bulkDelete.resource === "pods" ? t("bulkDelete.forcePodWarning") : t("bulkDelete.warning")}
              </p>
              <div className="bulk-delete-meta" aria-label="Bulk delete scope">
                <span>
                  {t("bulkDelete.resource")}: <strong>{bulkDelete.resource}</strong>
                </span>
                <span>
                  {t("bulkDelete.namespaces")}: <strong>{bulkDeleteNamespaceSummary(bulkDelete.rows)}</strong>
                </span>
              </div>
              <div className="bulk-delete-list-header">
                <span>{t("bulkDelete.resources")}</span>
                <button type="button" onClick={props.onCopyBulkDelete}>
                  {t("bulkDelete.copyList")}
                </button>
              </div>
            ...
            <footer>
              ...
              <button className="danger" onClick={props.onConfirmBulkDelete}>
                {t("common.delete")}
              </button>
            </footer>
          </section>
