...
Ветка `recovered/2.10.0-diagnostics-wip` не сливалась. Diagnostics Workspace
(kube-doctor, LLM-диагностика нод) решено не делать — продукту это не нужно;
ветка удалена.
...
- Шифрование LLM API key через Electron `safeStorage` — сделано в 2.10.1.
- Рефакторинг `App.tsx` и расширение `format:check` на директории — сделано
  отдельным chore-патчем.
- Подпись и нотаризация macOS — opt-in инфраструктура готова (electron-builder
  + entitlements + `docs/macos-signing.md`); сама подпись не выполнялась,
  требует Apple Developer Program membership.
- Изменение архитектуры хранения config — не запланировано.

Решено не делать:

- Linux `deb`, `rpm` и `arm64` — только AppImage x64.
- Подпись Windows и Linux сборок.
- macOS Intel — только arm64.
