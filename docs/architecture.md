...
Запрос заканчивается либо ответом, либо отменой: закрытие соединения до записи ответа снимает kubectl-процессы маршрутов с веером вызовов (списки ресурсов, search, problems, overview, related) и не даёт стартовать оставшимся. Общие кэши и фоновые задачи это переживают — подробности в `docs/api.md`.
...
Resource Snapshot Cache, история потребления подов, watch events, terminal, SSH и port-forward sessions хранятся только в памяти процесса. Каталог `metrics/` создаётся, но между запусками ничего не хранит: история существует в пределах одного запуска и ограничена окном в 24 часа, а файлы, оставшиеся от предыдущих версий, удаляются при старте.
...
`apps/desktop/src/shared` — модули, которые нужны обоим процессам. Main компилирует их в `dist/shared`, renderer вбирает в бандл. Сейчас там `formatQuantity.ts`, и он держит **два** формата величин, потому что их действительно два:

- дисплейный (`31.38 GiB`, `1.5 cores`, `250m`) — то, что читает человек: подписи, тайлы Overview, легенда графика, промпт LLM;
- нотация Kubernetes (`403840Ki`, `1500m`, `2`) — то, что печатает `kubectl top`, что хранит сэмплер и что показывает колонка usage; лимит рядом с показанием обязан быть в той же нотации, иначе одна полоса читается двумя способами.
...
Существующие HTTP/WebSocket маршруты принадлежат Node runtime. `/migration/status` сохранён как release diagnostic и должен сообщать `node-only`, `57 Node / 0 Python` для текущего contract baseline.
