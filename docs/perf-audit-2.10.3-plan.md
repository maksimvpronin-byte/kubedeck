...
- [x] Ручная проверка: запустить приложение, несколько раз переключить
  ...
  Выполнено пользователем локально на собранном приложении (2026-08-03,
  после push в `main`) — подтвердил, что всё работает. Это отчёт
  пользователя о ручном прогоне собранного приложения, а не пошаговое
  воспроизведение конкретного сценария (переключение табов + чтение
  `/watches/status`), которое я лично наблюдал бы напрямую — у меня самого
  нет доступа к реальному kubeconfig/кластеру в этой среде.
