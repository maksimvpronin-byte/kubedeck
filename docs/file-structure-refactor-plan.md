...
Статус: программа завершена. Восемь секций выпущены (2.20.1–2.20.8), находки закрыты (2.20.9, 2.20.10), расхождения с собственными критериями сведены (2.20.11). Ручная регрессия всех релизов пройдена владельцем проекта. Открытым осознанно остаётся один пункт — см. Секцию H и «Общий критерий».
...
- [x] Проверка эквивалентности до/после: **0 перевёрнутых пар, 0 потерянных,
  0 новых**.
- [x] Регрессия — ручная, обязательна и должна попасть в
...
- [x] Регрессия: каждая кнопка приложения на всех 8 темах.
...
Ручная регрессия всех релизов 2.20.1–2.20.11 пройдена: чек-листы в
`docs/releases/` отмечены. Единственный релиз с неотмеченным чек-листом —
2.20.0, он предшествует этой программе.
