...
Статус: все восемь секций пройдены (2.20.1–2.20.8). Две цели Секции H остались недостижимы разбором — см. её раздел.
...
(Секция B). Порог «меньше 60 `!important`» **не достигнут и снят как
недостижимый разбором** — 227 вместо 60; вместо запрета линию держит храповик
`npm run lint:css` (Секция H). Ни один тестовый файл
