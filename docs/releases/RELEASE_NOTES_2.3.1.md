...
- Минимальная ширина меню namespace равна ширине селектора, а максимальная автоматически определяется самым длинным именем namespace.
- Имена отображаются целиком в одну строку без обрезки и переноса; системная подсказка при наведении доступна дополнительно.
