...
- [ ] **Status/phase** - the word, its colour, and the tooltip naming the
  reason. A Running pod that is fully ready has no tooltip; one at 1/2 does.
- [ ] A **Pending** pod reads pending, not red; a **CrashLoopBackOff** pod reads
  red.
- [ ] **Container cubes** on a pod row: one per container, tooltip naming the
...
- [ ] A pod whose row knows only container names still shows one grey cube each.
- [ ] **Age** ticks forward once a second, and a pod older than a day does not
  flicker.
- [ ] **Node usage** - CPU, RAM and Disk bars, with the disk one showing `…`
  while it loads and `N/A` when there is none.
- [ ] **Namespace usage** - CPU, RAM and Storage against quota, and `No quota`
  where none is set.
- [ ] **Pod usage** - the bar against a limit where there is one; against the
...
- [ ] **Workload conditions** on a deployment: the chips, their colours and the
  tooltip with the replica summary.
- [ ] **Node labels, roles and annotations** cells, including clicking one to
  filter.
- [ ] Sorting by a usage column still offers its metrics and sorts by them.

## The Problems panel

- [ ] The summary bar counts, the filters (severity, namespace, kind,
  category), and the table below.
- [ ] The **guidance block**: groups of problems with a count each, most severe
  first, at most four.
- [ ] The **priority list**: each card names the resource, the reason and the
  next check.
- [ ] **Open** a problem that is an Event about a pod: the pod opens, not the
  event.
- [ ] **Open** a problem that is the pod itself: the pod opens.
- [ ] **Copy** a problem: the clipboard text carries cluster, severity,
  category, resource, reason, message and next check - and names the reporting
  event when it differs from the target.
- [ ] A cluster with no problems shows the empty state.
- [ ] Resizing the window with the drawer open still reflows the panel by its
  own width (the 2.19.x container-query behaviour).

## Nothing else moved

- [ ] Overview, Global Search, the resource drawer, Settings.
- [ ] Run an **LLM** analysis on a pod: no Secret value or log line reaches the
  prompt.
- [ ] Switch themes and languages.
- [ ] Help and About report **2.20.6**.
