...
**Поправка после самопроверки "всё ли сделано по плану":** аудит (и я при
реализации Секции A) пропустили три реальных дубликата того же паттерна:
`routes/watch.ts` и `routes/portForward.ts` имели собственный локальный
`decodePart`, не сведённый к общему `decodePathPart`; `routes/llm.ts`
по-прежнему объявлял локальный `LlmRequestError`, хотя чек-бокс "убрать
LlmRequestError" был отмечен выполненным. Все три исправлены отдельным
коммитом после повторной ручной проверки (`grep` по всему `backend/` на
`^function decodePart`/`^function writeRouteError`/`readonly statusCode:
number,`), gate зелёный, версия не менялась.
