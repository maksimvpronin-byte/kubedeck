# KubeDeck 2.0 — архив промежуточной миграции на Node

> Исторический документ состояния `2.0.0-alpha.5`. Миграция завершена: актуальный runtime является Node-only. Текущее состояние описано в корневом `NODE_MIGRATION_PROGRESS.md`, `docs/architecture.md` и `docs/security.md`.
